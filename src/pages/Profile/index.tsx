...
const SupporterView: React.FC = () => (
  <section className="space-y-6 rounded-3xl border border-neutral-200 p-6 text-sm text-neutral-700">
    ...
    <p className="text-xs text-neutral-500">실제 결제 내역이 연동되면 여기에서 상태를 확인할 수 있습니다.</p>
    <div className="rounded-2xl border border-neutral-100 p-4 text-neutral-600">아직 후원한 프로젝트가 없습니다.</div>

    <div className="space-y-2">
      <h3 className="text-sm font-semibold text-neutral-900">관심 프로젝트</h3>
      <p className="text-xs text-neutral-500">찜한 프로젝트가 표시됩니다. 프로젝트 상세에서 하트를 눌러 저장해 보세요.</p>
...
const stageMeta: Array<{
  ...
  { status: "DRAFT", title: "작성 중", description: "아직 제출하지 않은 초안 프로젝트", link: "/creator/projects/new" },
  { status: "REVIEW", title: "심사 중", description: "승인 대기 중인 프로젝트", link: "/creator/dashboard" },
  { status: "APPROVED", title: "승인됨", description: "검토를 통과해 공개를 준비 중입니다.", link: "/creator/dashboard" },
  { status: "SCHEDULED", title: "공개 예정", description: "LIVE 예정일을 기다리고 있는 프로젝트", link: "/creator/dashboard" },
  { status: "LIVE", title: "진행 중", description: "현재 후원받고 있는 프로젝트", link: "/creator/dashboard" },
  { status: "ENDED", title: "종료", description: "마감된 프로젝트 및 정산을 확인하세요.", link: "/creator/dashboard" },
  { status: "REJECTED", title: "반려됨", description: "수정 후 다시 제출이 필요한 프로젝트", link: "/creator/dashboard" },
];
...
const MakerView: React.FC<{ displayName: string; drafts: ProjectDraft[] }> = ({ displayName, drafts }) => {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-neutral-200 p-6 text-sm text-neutral-700">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-neutral-900">메이커 대시보드</h2>
            <p className="text-xs text-neutral-500">저장한 프로젝트와 진행 상황을 한눈에 확인하세요.</p>
          </div>
          <Link
      ...
      <section className="space-y-3 rounded-3xl border border-neutral-200 p-6 text-sm text-neutral-700">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-neutral-900">내 프로젝트</h2>
          <Link
        ...
        <p className="text-xs text-neutral-500">상태별로 프로젝트를 살펴보고 필요한 작업을 이어서 진행하세요.</p>
          ...
          {stageMeta.map((stage) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-neutral-400">{stageCounts[stage.status]?.toLocaleString?.() ?? 0}개</span>
              </div>
      ...
      <section className="space-y-3 rounded-3xl border border-neutral-200 p-6 text-sm text-neutral-700">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-neutral-900">프로필 카드</h2>
        ...
        <div className="rounded-2xl border border-neutral-100 p-5">
          ...
          <h3 className="mt-2 text-lg font-semibold text-neutral-900">{displayName || "새로운 메이커"}</h3>
          <p className="mt-2 text-sm text-neutral-600">
            소개 글을 작성하면 후원자에게 더 많은 신뢰를 줄 수 있습니다. 제작 배경이나 목표를 간단히 적어 주세요.
          </p>
          <div className="mt-4 space-y-1 text-xs text-neutral-500">
            <p>홈페이지: example.com</p>
            <p>소셜 링크: instagram.com/username</p>
            <p>상태별 프로젝트: {Object.entries(stageCounts)
              .filter(([, count]) => count > 0)
              .map(([status, count]) => `${statusLabel[status as ProjectDraft["status"]]} ${count}개`)
              .join(", ") || "등록된 프로젝트가 없습니다."}</p>
          </div>
        </div>
      </section>
    </div>
...
export const ProfilePage: React.FC = () => {
  ...
  const [mode, setMode] = React.useState<"supporter" | "maker">("supporter");

  return (
    <Container>
      <div className="mx-auto flex min-h-[70vh] max-w-4xl flex-col gap-6 py-16">
        <header className="space-y-2">
          <h1 className="text-2xl font-semibold text-neutral-900">마이페이지</h1>
          <p className="text-sm text-neutral-500">서포터와 메이커 역할을 전환하며 필요한 정보를 빠르게 살펴보세요.</p>
        </header>
        ...
        <div className="flex items-center gap-2 rounded-full border border-neutral-200 p-1 text-sm text-neutral-600">
          <button
            ...
            className={`flex-1 rounded-full px-4 py-2 transition ${
              mode === "supporter" ? "bg-neutral-900 text-white" : "hover:text-neutral-900"
            }`}
          >
            서포터 보기
          </button>
          <button
            ...
            className={`flex-1 rounded-full px-4 py-2 transition ${
              mode === "maker" ? "bg-neutral-900 text-white" : "hover:text-neutral-900"
            }`}
          >
            메이커 보기
          </button>
        ...
        {mode === "supporter" ? (
          ...
          <MakerView displayName={user?.name ?? "크리에이터"} drafts={drafts} />
        )}
      </div>
    </Container>
  );
