    ...
    <Container>
      <div className="space-y-8 py-16">
        <header className="space-y-2">
          <h1 className="text-2xl font-semibold text-neutral-900">크리에이터 대시보드</h1>
          <p className="text-sm text-neutral-500">
            {user?.name ?? "크리에이터"}님의 프로젝트 성과를 한눈에 확인하고, 프로젝트별 리워드
            판매 현황을 살펴보세요.
          </p>
        </header>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            {metrics.map((metric) => (
              <div key={metric.label} className="rounded-3xl border border-neutral-200 p-4">
        ...
        <section className="space-y-4 rounded-3xl border border-neutral-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-neutral-900">리워드별 판매 현황</h2>
              <p className="text-xs text-neutral-500">프로젝트별 리워드 판매·환불·재고 정보를 확인하세요.</p>
            </div>
                  ...
                  <React.Fragment key={project.project}>
                    <tr className="bg-neutral-50 text-xs uppercase tracking-[0.12em] text-neutral-500">
                      <td className="py-3 pr-6" colSpan={7}>
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`rounded-full px-2 py-1 text-[10px] ${statusBadge[project.status] ?? "bg-neutral-200 text-neutral-600"}`}>
                            {project.status}
                          </span>
                        </div>
                      </td>
                    ...
                    {project.rewards.map((reward) => (
                      <tr key={`${project.project}-${reward.name}`}>
                        ...
                        <td className="py-3 pr-6 text-neutral-900">{reward.name}</td>
                        <td className="py-3 pr-6">{formatKRW(reward.price)}</td>
                        <td className="py-3 pr-6">{reward.sold.toLocaleString()}</td>
                        <td className="py-3 pr-6">{reward.refunded.toLocaleString()}</td>
                        <td className="py-3 pr-6">{reward.stock.toLocaleString()}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                ))}
